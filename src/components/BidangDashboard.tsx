        ...
        <div className="bg-white rounded-2xl shadow-md border border-slate-200 overflow-hidden mb-6">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {/* INTEGRASI USULAN MUSRENBANG DESA / KELURAHAN SECTION */}
              {formData.sumberUsulan === 'Musrenbang Desa / Kelurahan' && (
                <div className="md:col-span-3 bg-emerald-50/90 border border-emerald-300 rounded-2xl p-4 space-y-3">
                  <div className="flex items-center justify-between border-b border-emerald-200 pb-2">
                    <div className="flex items-center gap-2">
                      <MapPin className="w-4 h-4 text-emerald-800" />
                      <h4 className="text-xs font-black text-emerald-950 uppercase tracking-wider">
                        Integrasi Hasil Musrenbang Desa / Kelurahan (RKPDes)
                      </h4>
                    </div>
                    <span className="text-[10px] font-extrabold text-emerald-900 bg-emerald-100 px-2.5 py-0.5 rounded-full border border-emerald-300">
                      Terhubung Musrenbang Kecamatan & RENJA OPD
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs">
                    <div>
                      <label className="text-[11px] font-bold text-emerald-900 block mb-1">
                        Nomor Berita Acara RKPDes / Musrenbang Desa
                      </label>
                      <input
                        type="text"
                        placeholder="Contoh: 01/BA-DESA/MRPKT/2026"
                        value={formData.nomorMusrenbangDesa}
                        onChange={e => setFormData({ ...formData, nomorMusrenbangDesa: e.target.value })}
                        className="w-full border border-emerald-300 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 bg-white focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    </div>

                    <div>
                      <label className="text-[11px] font-bold text-emerald-900 block mb-1">
                        Rangking Prioritas Usulan Desa (RKPDes)
                      </label>
                      <input
                        type="text"
                        placeholder="Contoh: Prioritas #1 RKPDes"
                        value={formData.prioritasDesa}
                        onChange={e => setFormData({ ...formData, prioritasDesa: e.target.value })}
                        className="w-full border border-emerald-300 rounded-xl px-3 py-2 text-xs font-bold text-slate-900 bg-white focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    </div>
                    ...
                    <div>
                      <label className="text-[11px] font-bold text-emerald-900 block mb-1">
                        Link Dokumen / Berita Acara Desa (Drive/PDF)
                      </label>
                      <input
                        type="text"
                        placeholder="https://drive.google.com/file/d/..."
                        value={formData.beritaAcaraDesaUrl}
                        onChange={e => setFormData({ ...formData, beritaAcaraDesaUrl: e.target.value })}
                        className="w-full border border-emerald-300 rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 bg-white focus:ring-2 focus:ring-emerald-500 outline-none"
                      />
                    </div>
                  </div>

                  <div className="text-[11px] font-semibold text-emerald-900 bg-emerald-100/70 p-2.5 rounded-xl border border-emerald-200 flex items-center gap-2">
                    <CheckCircle className="w-4 h-4 text-emerald-700 shrink-0" />
                    <span>
                      Usulan Desa ini terhubung ke lokasi: <strong>{selectedDesaList.length > 0 ? selectedDesaList.join(', ') : (formData.desa || 'Seluruh Desa')}</strong> (Kec. {selectedKecList.length > 0 ? selectedKecList.join(', ') : (formData.kecamatan || 'Nagekeo')}). Usulan akan diteruskan ke Musrenbang Kecamatan & RENJA OPD di SIPD RI.
                    </span>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-2xl w-full p-6 shadow-2xl space-y-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs">
              ...
              {/* DETAIL MUSRENBANG DESA */}
              {(selectedProposalDetail.sumberUsulan?.includes('Desa') || selectedProposalDetail.nomorMusrenbangDesa) && (
                <div className="col-span-2 bg-emerald-50 border border-emerald-200 p-3 rounded-xl space-y-1">
                  <p className="font-extrabold text-emerald-950 flex items-center gap-1.5 text-xs">
                    <MapPin className="w-4 h-4 text-emerald-700" /> Hasil Integrasi Musrenbang Desa / Kelurahan (RKPDes)
                  </p>
                  <div className="grid grid-cols-2 gap-2 text-[11px] pt-1">
                    <div>
                      <span className="text-emerald-800 font-semibold">No. BA RKPDes / Musrenbang:</span>
                      <p className="font-extrabold text-slate-900">{selectedProposalDetail.nomorMusrenbangDesa || '-'}</p>
                    </div>
                    <div>
                      <span className="text-emerald-800 font-semibold">Ranking Prioritas Desa:</span>
                      <p className="font-extrabold text-emerald-900">{selectedProposalDetail.prioritasDesa || '-'}</p>
                    </div>
                  </div>
                  {selectedProposalDetail.beritaAcaraDesaUrl && (
                    <div className="pt-1 text-[11px]">
                      <a href={selectedProposalDetail.beritaAcaraDesaUrl} target="_blank" rel="noopener noreferrer" className="text-emerald-700 font-bold underline flex items-center gap-1 hover:text-emerald-900">
                        📄 Lihat Dokumen Berita Acara Musrenbang Desa
                      </a>
                    </div>
                  )}
                </div>
